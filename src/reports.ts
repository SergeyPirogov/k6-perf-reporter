...
export class ReportGenerator {
  constructor(private influxClient: InfluxQueryClient) {}
  ...
  async generateMetrics(
    ...
    console.log(
      chalk.blue(`  📍 Fetching metrics for scenario: ${chalk.bold(scenario)}`)
    );
    console.log(chalk.gray(`     Time range: ${startTime} to ${endTime}`));

    try {
      console.log(chalk.gray("     ├─ Querying response time percentiles..."));
      const startPercentilesTime = Date.now();
      const percentiles = await this.influxClient.getResponseTimePercentiles(
        scenario,
        startTime,
        endTime
      );
      ...
      console.log(
        chalk.green(
          `     ├─ ✓ Response times: P50=${percentiles.p50.toFixed(2)}ms, P95=${percentiles.p95.toFixed(2)}ms, P99=${percentiles.p99.toFixed(2)}ms (${percentilesDuration}ms)`
        )
      );

      console.log(chalk.gray("     ├─ Querying request statistics..."));
      const startStatsTime = Date.now();
      const stats = await this.influxClient.getRequestStats(
        scenario,
        startTime,
        endTime
      );
      ...
      console.log(
        chalk.green(
          `     ├─ ✓ Requests: total=${stats.total}, success=${stats.success}, failed=${stats.failed} (${statsDuration}ms)`
        )
      );

      console.log(chalk.gray("     └─ Querying error breakdown..."));
      const startErrorsTime = Date.now();
      const errors = await this.influxClient.getErrorBreakdown(
        scenario,
        startTime,
        endTime
      );
      const errorsDuration = Date.now() - startErrorsTime;
      const errorCount = Array.from(errors.values()).reduce(
        (sum, count) => sum + count,
        0
      );
      console.log(
        chalk.green(
          `     └─ ✓ Errors: ${errors.size} unique error types, ${errorCount} total errors (${errorsDuration}ms)`
        )
      );
      ...
      const durationSeconds =
        (endDate.getTime() - startDate.getTime()) / 1000 || 1;
      const throughput = stats.total / durationSeconds;

      console.log(
        chalk.blue(
          `  📊 Metrics summary: Error rate=${errorRate.toFixed(2)}%, Throughput=${throughput.toFixed(2)} req/s`
        )
      );
      ...
      console.error(
        chalk.red("  ❌ Error fetching metrics:"),
